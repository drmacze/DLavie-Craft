(() => {
  ...
  let observer=null,raf=0,allowed=false,checked=false,active=false,stickers=[],busy=false,previewUrl='';
  ...
  async function upload(file,name){
    ...
    }catch(e){throw new Error(`File terunggah tetapi metadata gagal disimpan: ${e.message}`);}
  }

  function stats(){const activeCount=stickers.filter(s=>s.active).length,gif=stickers.filter(s=>s.animated).length;return {activeCount,gif,total:stickers.length};}
  function renderPanel(){
    const content=document.querySelector('.console-app .console-content');if(!content||!active)return;
    let panel=content.querySelector(':scope > .dl-console-sticker-panel');if(panel)panel.remove();
    panel=document.createElement('section');panel.className='dl-console-sticker-panel';
    const s=stats();
    ...
    form.onsubmit=async e=>{e.preventDefault();if(busy)return;const file=fileInput.files?.[0],name=form.elements.name.value.trim();if(!file||!name)return;busy=true;submit.disabled=true;submit.textContent='Mengunggah…';try{await upload(file,name);core().toast?.('Sticker berhasil ditambahkan');await load();if(previewUrl){URL.revokeObjectURL(previewUrl);previewUrl='';}renderPanel();window.__DLAVIE_PUBLIC_STICKERS__?.reload?.();}catch(err){core().toast?.(err.message);}finally{busy=false;}};
    const list=panel.querySelector('.dl-console-sticker-list');
    if(!stickers.length){list.innerHTML='<div class="dl-console-sticker-empty">Belum ada sticker. Upload sticker pertama untuk komunitas.</div>';}
    stickers.forEach(item=>{
      const row=document.createElement('article');row.className=`dl-console-sticker-row${item.active?' is-active':''}`;
      row.innerHTML=`<img src="${esc(item.public_url)}" alt="${esc(item.name)}" loading="lazy" decoding="async"><div><strong>${esc(item.name)}</strong><small>${item.animated?'GIF':String(item.mime_type||'image').replace('image/','').toUpperCase()} · ${Math.max(1,Math.round(Number(item.file_size||0)/1024))} KB</small></div>`;
      const toggle=button(item.active?'Aktif':'Nonaktif','dl-console-sticker-toggle');toggle.setAttribute('aria-pressed',String(item.active));
      toggle.onclick=async()=>{if(busy)return;busy=true;toggle.disabled=true;try{await core().api(`dlavie_craft_community_stickers?id=eq.${encodeURIComponent(item.id)}`,{method:'PATCH',write:true,body:JSON.stringify({active:!item.active,updated_at:new Date().toISOString()})});item.active=!item.active;core().toast?.(item.active?'Sticker diaktifkan':'Sticker dinonaktifkan');renderPanel();}catch(err){core().toast?.(err.message);}finally{busy=false;}};
      row.append(toggle);list.append(row);
    });
    content.append(panel);
  }

  async function openStudio(){active=true;document.querySelector('.console-app')?.classList.add('dl-console-sticker-active');document.querySelector('.console-sidebar')?.classList.remove('open');document.querySelectorAll('.console-sidebar nav button').forEach(b=>b.classList.toggle('active',b.dataset.dlStickerConsole==='1'));await load();renderPanel();}
  ...
  function ensure(){
    if(!CONSOLE_ROUTE.test(location.hash)||!allowed)return;
    const nav=document.querySelector('.console-app .console-sidebar nav');if(!nav)return;
    ...
    if(active){document.querySelector('.console-app')?.classList.add('dl-console-sticker-active');renderPanel();}
  }
  function schedule(){if(raf)return;raf=requestAnimationFrame(()=>{raf=0;ensure();});}
  async function route(){
    if(!CONSOLE_ROUTE.test(location.hash)){closeStudio();observer?.disconnect();observer=null;checked=false;allowed=false;return;}
    checked=false;if(!await canManage())return;
    let tries=0;const wait=()=>{const app=document.querySelector('.console-app');if(!app&&tries++<40)return setTimeout(wait,90);if(!app)return;ensure();observer?.disconnect();observer=new MutationObserver(records=>{const meaningful=records.some(r=>{const t=r.target?.nodeType===1?r.target:r.target?.parentElement;return !t?.closest?.('.dl-console-sticker-panel,[data-dl-sticker-console]');});if(meaningful)schedule();});observer.observe(app,{childList:true,subtree:true});};wait();
  }
