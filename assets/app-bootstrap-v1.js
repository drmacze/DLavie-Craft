(function(){
  ...
  var loaded={};
  var loadEventFired=document.readyState==='complete';
  var gamehubStarted=false,accountStarted=false,communityStarted=false,motionStarted=false;
  ...
  function series(list){
    return list.reduce(function(p,x){return p.then(function(){return script(x)})},Promise.resolve(true));
  }
  ...
  function loadAccount(){
    ...
    accountStarted=true;
    accountCss.forEach(css);
    series(accountJs);
  }
  function loadCommunity(){
    if(communityStarted)return;
    communityStarted=true;
    communityCss.forEach(css);
    series(communityJs);
  }
  function route(){
    var h=location.hash||'';
    if(/account|profile|login|register/i.test(h))loadAccount();
    if(/community/i.test(h))loadCommunity();
  }

  function afterInitialLoad(){
    if(window.__DLAVIE_WELCOME_ACTIVE__){
      window.addEventListener('dlavie:welcome-complete',function(){setTimeout(startGamehub,40)},{once:true});
    }else{
      setTimeout(startGamehub,40);
    }
    route();
  }

  window.addEventListener('hashchange',route);
  if(loadEventFired)afterInitialLoad();
  else window.addEventListener('load',function(){loadEventFired=true;afterInitialLoad()},{once:true});

  function loadMotion(){
    if(motionStarted)return;
    motionStarted=true;
    script('https://cdn.jsdelivr.net/npm/gsap@3.13.0/dist/gsap.min.js')
      .then(function(){return script('https://cdn.jsdelivr.net/npm/gsap@3.13.0/dist/ScrollTrigger.min.js')})
      .then(function(){return script('https://cdn.jsdelivr.net/npm/lenis@1.3.11/dist/lenis.min.js')})
      .then(function(){return script(A+'dlavie-motion-engine-v2.js?v=20260905q2')});
  }

  // External motion libraries are no longer part of the refresh critical path.
  // Load them only after the user actually interacts with the page.
  ['pointerdown','touchstart','wheel','keydown'].forEach(function(name){
    window.addEventListener(name,loadMotion,{once:true,passive:true});
  });
})();